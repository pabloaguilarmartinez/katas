...
describe('In the game of life', () => {
  it('any live cell with fewer than two live neighbours dies, as if caused by underpopulation', () => {
    ...
    expect(Cell.create('ALIVE').regenerate(numberOfNeighbours).isAlive()).toBeFalsy();
    expect(Cell.create('DEAD').regenerate(numberOfNeighbours).isAlive()).toBeFalsy();
  });
  ...
  it('any live cell with two or three live neighbours lives on to the next generation', () => {
    expect(Cell.create('ALIVE').regenerate(2).isAlive()).toBeTruthy();
    expect(Cell.create('ALIVE').regenerate(3).isAlive()).toBeTruthy();
    expect(Cell.create('DEAD').regenerate(2).isAlive()).toBeFalsy();
  });
  it('any dead cell with exactly three live neighbours becomes a live cell', () => {
    expect(Cell.create('DEAD').regenerate(3).isAlive()).toBeTruthy();
  });
